...
public class StartEvent {
    public static void StartEvent() {
        ...
        CommandRegistrationCallback.EVENT.register((commandDispatcher, commandRegistryAccess, registrationEnvironment) ->
                commandDispatcher.register(CommandManager.literal("resetevent").requires(serverCommandSource -> serverCommandSource.hasPermissionLevel(2))
                        .executes(commandContext -> {
                            if (!commandContext.getSource().getPlayer().getWorld().isClient()) {
                                commandContext.getSource().getPlayer().getServerWorld().getPlayers().forEach(serverPlayerEntity -> {
                                    serverPlayerEntity.networkHandler.sendPacket(new TitleS2CPacket(Text.of("§4||§r§6 The event has been reset. §r§4||§r")));
                                    serverPlayerEntity.getInventory().clear();
                                    SkinShifter.setSkin(serverPlayerEntity, serverPlayerEntity.getUuid());
                                    serverPlayerEntity.getAttributeInstance(EntityAttributes.GENERIC_SCALE).setBaseValue(1.0d);
                                });
                            }
                            return 0;
                        })));
